...
export function GlowingButton({ title, onClick }: { title: string, onClick?: () => void }) {
    return (
        <div className="relative group inline-block">
            ...
            {/* Actual button */}
            <button onClick={onClick} className="relative bg-[#0d0c1b] text-white px-4 py-1.5 rounded-full border border-gray-700 hover:bg-[#1a1a2e] transition-all duration-300">
                {title}
            </button>
        </div>
    );
