    ...
    return (
        <section className="md:px-20 py-10">
            <footer className="bg-gradient-to-r to-red-500 from-purple-600 text-white px-8 md:px-16 lg:px-32 py-10 flex flex-col justify-between items-start shadow-lg rounded-lg">
                ...
                <div className="flex sm:flex-row flex-col sm:items-end justify-between w-full h-fit">
                    {/* Footer Items */}
                    <div className="flex flex-col">
                        <h3 className="text-xl font-semibold mt-4">Quick Links</h3>
                        <hr className="w-full h-[0.1px] bg-white mt-1" />
                        <ul className="sm:space-y-2 md:space-y-1 pl-1 pt-4">
                            {footer.footerItems.map((item, index) => (
                                <li key={index}>
                                    <a href={item.link} className="hover:underline text-sm">{item.label}</a>
                                </li>
                            ))}
                    ...
                    <div className="flex items-center sm:justify-center justify-around sm:gap-8 gap-5 mt-5 sm:w-auto w-full">
                        <Link href={`https://x.com/clipcurve_x`} target="_blank">
                            <FaSquareXTwitter className="sm:text-3xl text-2xl text-black" />
                        </Link>
                        <Link href={`https://www.linkedin.com/company/clip-curve`} target="_blank">
                            <FaLinkedin className="text-blue-500 sm:text-3xl text-2xl" />
                        </Link>
                        <Link href={`mailto:theclipcurve@gmail.com`} target="_blank">
                            <SiGmail className="text-[#f2a60c] sm:text-3xl text-2xl" />
                        </Link>
                    </div>
                    <div className="flex flex-col items-end justify-end h-full gap-2 mb-2">
                        ...
                        <Link href={"/terms&conditions/privacy-policy"} className=" text-sm md:mt-0">Privacy Policy</Link>
                        <Link href={"/terms&conditions"} className=" text-sm md:mt-0">Terms and Conditions</Link>
                        <Link href={"/terms&conditions/dssPolicy"} className=" text-sm md:mt-0">Digital Services Shipping Policy</Link>
                        <Link href={"/terms&conditions/carPolicy"} className=" text-sm md:mt-0">Cancellation and Refund Policy</Link>
